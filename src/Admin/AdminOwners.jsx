...
const AdminOwners = () => {
    ...
    useEffect(() => {
        const fetchOwners = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/owner/getallowners');
                setOwners(response.data);
                setFilteredOwners(response.data); // Initialize filteredOwners with the complete list
            } catch (error) {
                console.error('Error fetching owner data:', error);
            }
        };
    ...
    const confirmDelete = async () => {
        try {
            await axios.delete(`http://localhost:8080/api/owner/delete/${ownerToDelete}`);
            setOwners(owners.filter(owner => owner.ownerId !== ownerToDelete));
            setFilteredOwners(filteredOwners.filter(owner => owner.ownerId !== ownerToDelete));
            setshowdelPopup(false);
            toast.success('You have deleted the owner successfully!');
        } catch (error) {
            console.error('Error deleting owner:', error);
    ...
    const handleInputChange = (e) => {
        ...
        if (name === 'ownerPhone') {
            // Allow only numbers and enforce length of 10
            const phoneNumber = value.replace(/\D/g, ''); // Remove non-digit characters
            if (phoneNumber.length <= 10) {
                setNewOwner(prevState => ({ ...prevState, ownerPhone: phoneNumber }));
            }
        } else if (name === 'cityId') {
            // Allow only numbers from 1 to 6
            const cityId = value.replace(/\D/g, ''); // Remove non-digit characters
            if (/^[1-6]$/.test(cityId) || cityId === '') {
                setNewOwner(prevState => ({
                    ...prevState,
                    city: { ...prevState.city, cityId: cityId }
                }));
            }
        } else {
            setNewOwner(prevState => ({ ...prevState, [name]: value }));
    ...
    const handleAddOwner = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:8080/api/owner/add', newOwner);
            setOwners([...owners, response.data]);
            setFilteredOwners([...owners, response.data]);
            setShowPopup(false);
            setNewOwner({
                ownerName: '',
                ownerEmail: '',
